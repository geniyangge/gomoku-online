...
export interface Room {
  id: string
  name: string
  players: [string | null, string | null] // 两个位置的玩家ID
  spectators: string[] // 观众ID列表
  status: GameStatus
  board: number[][]
  currentPlayer: number // 0或1，表示当前轮到谁
  winner: string | null
  drawRequests: DrawRequest[]
  drawCount: Record<string, number> // 每个玩家的和局请求次数
  settlementEndTime: number | null // 结算结束时间
...
export interface ServerToClientEvents {
  ...
  'room:joined': (data: {
    ...
    restored?: boolean
    gameState?: { status: GameStatus; playerIndex: number | null; roomId: string | null }
  }) => void
